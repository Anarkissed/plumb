...
let selfTestGrid = UserDefaults.standard.bool(forKey: "selftestGrid")

let app = NSApplication.shared

if selfTestGrid {
    // Grid-tiling self-test: activates a target app (Finder by default), enumerates its windows,
    // and places them into an N-cell grid via the real service — logging before/after frames.
    UserDefaults.standard.set(false, forKey: "selftestGrid")
    app.setActivationPolicy(.regular)
    app.delegate = SelfTestGridDelegate()
    app.run()
    exit(0)
}
