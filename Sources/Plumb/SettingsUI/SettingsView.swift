...
struct SettingsView: View {
    ...
    let gridStore: GridTilingSettingsStore
    @State private var settings: AppTilingSettings
    @State private var gridSettings: GridTilingSettings
    ...
    enum Section: Hashable, CaseIterable {
        case centering, tiling, grid, permissions, about
        var title: String {
            switch self {
            case .centering: return L10n.tabCentering
            case .tiling: return L10n.tabTiling
            case .grid: return "Grid"           // 网格平铺标签（暂英文硬编码，避免改本地化表）。
    ...
    init(store: AppTilingSettingsStore, gridStore: GridTilingSettingsStore) {
        self.store = store
        self.gridStore = gridStore
        _settings = State(initialValue: store.load())
        _gridSettings = State(initialValue: gridStore.load())
    }
    ...
    var body: some View {
        ...
        .onChange(of: gridSettings) { _, new in
            DiagnosticLog.debug("SettingsUI: grid settings changed → save")
            gridStore.save(new)
        }
    ...
    private var detailView: some View {
        ...
        case .grid:
            GridSection(settings: $gridSettings, apps: apps)
        case .permissions:
