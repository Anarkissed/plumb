import AppKit
import Carbon.HIToolbox   // kVK_ANSI_T / cmdKey / optionKey（Tile Now 全局热键）
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    private let gridSettingsStore = GridTilingSettingsStore()
    private let dmgMonitor = DmgMountMonitor()
    /// 网格平铺编排器（受管集合 + 区域网格 + AX 写入）。与 Observer 共用同一个 centeringService。
    private lazy var gridController = GridTilingController(
        service: centeringService,
        store: gridSettingsStore
    )
    ...
    /// 「Tile now」全局热键（⌥⌘T）。强引用保活；nil 表示注册失败（组合被占用）。
    private var tileNowHotKey: GlobalHotKey?
    /// 「Auto-tile 前台 app 开关」全局热键（⌥⌘G）：一键把当前 app 加入/移出网格受管列表。
    private var toggleManagedHotKey: GlobalHotKey?
    /// 「Tile Layout」子菜单控制器（可视化布局选择 + hover 预览）。强引用保活。
    private var tileLayoutMenuController: TileLayoutMenuController?
    ...
    private func finishLaunchingAfterStatusIconRegistration() {
        ...
        dmgMonitor.start()
        eventObserver.start()
        registerTileNowHotKey()
        registerToggleManagedHotKey()
        centerOnceOnLaunch()
    ...
    /// 「Tile Now」菜单项 / 全局热键（⌥⌘T）：把前台 app 当前全部窗口一次排成网格。
    @objc private func tileNowAction() {
        gridController.tileNowFrontmost()
    }

    /// 「Auto-tile new windows」菜单项：翻转前台 app 的网格受管标记（持久）。
    /// 勾选后该 app 之后新开的窗口自动加入网格；已开窗口仍需 Tile Now 才纳入。
    @objc private func toggleGridManaged() {
        guard let app = NSWorkspace.shared.frontmostApplication,
              let bundleID = app.bundleIdentifier else { return }
        let norm = AppTilingSettings.normalizeBundleID(bundleID)
        var cfg = gridSettingsStore.load()
        let nowOn: Bool
        if cfg.managedBundleIDs.contains(norm) {
            cfg.managedBundleIDs.remove(norm)
            nowOn = false
        } else {
            cfg.managedBundleIDs.insert(norm)
            nowOn = true
        }
        gridSettingsStore.save(cfg)
        DiagnosticLog.debug("grid: toggled auto-tile for \(app.localizedName ?? norm) → \(nowOn ? "ON" : "OFF")")
        // 通知打开中的设置窗口重载网格设置（菜单→设置窗口同步，与两个总开关同机制）。
        NotificationCenter.default.post(name: SettingsWindowNotifications.settingsChangedExternally, object: nil)
    }

    /// 注册「切换前台 app 网格受管」全局热键 ⌥⌘G。失败（组合被占用）时仅记录日志，菜单勾选项仍可用。
    private func registerToggleManagedHotKey() {
        toggleManagedHotKey = GlobalHotKey(
            keyCode: UInt32(kVK_ANSI_G),
            modifiers: UInt32(cmdKey | optionKey)
        ) { [weak self] in
            MainActor.assumeIsolated {
                self?.toggleGridManaged()
            }
        }
        if toggleManagedHotKey == nil {
            DiagnosticLog.debug("hotkey: failed to register ⌥⌘G for toggle-managed (combo taken?)")
        }
    }

    /// 注册「Tile now」全局热键 ⌥⌘T。失败（组合被占用）时仅记录日志，菜单项仍可用。
    private func registerTileNowHotKey() {
        tileNowHotKey = GlobalHotKey(
            keyCode: UInt32(kVK_ANSI_T),
            modifiers: UInt32(cmdKey | optionKey)
        ) { [weak self] in
            MainActor.assumeIsolated {
                _ = self?.gridController.tileNowFrontmost()
            }
        }
        if tileNowHotKey == nil {
            DiagnosticLog.debug("hotkey: failed to register ⌥⌘T for Tile Now (combo taken?)")
        } else {
            DiagnosticLog.debug("hotkey: registered ⌥⌘T for Tile Now")
        }
    }
    ...
    func menuNeedsUpdate(_ menu: NSMenu) {
        ...
        let grid = gridSettingsStore.load()
        let frontApp = NSWorkspace.shared.frontmostApplication
        let frontBundle = frontApp?.bundleIdentifier.map(AppTilingSettings.normalizeBundleID)
        for item in menu.items {
            ...
            case #selector(toggleGridManaged):
                // 动态标题带上前台 app 名（如「Auto-tile Finder's new windows」），
                // 勾选 = 该 app 已在网格受管列表内。无前台 app 时置灰。
                if let frontApp, let frontBundle {
                    let name = frontApp.localizedName ?? "this app"
                    item.title = "Auto-tile \(name)'s new windows"
                    item.state = grid.managedBundleIDs.contains(frontBundle) ? .on : .off
                    item.isEnabled = true
                } else {
                    item.title = "Auto-tile new windows"
                    item.state = .off
    ...
    @objc private func openSettings() {
        if settingsWindowController == nil {
            settingsWindowController = SettingsWindowController(store: tilingSettingsStore, gridStore: gridSettingsStore)
        }
    ...
    private func showStatusBarIcon() {
        ...
        // 网格平铺（Grid tiling）：把多个窗口铺成网格。
        //   - Tile Now（⌥⌘T，也有全局热键）：把前台 app 当前全部窗口一次排成网格。
        //   - Auto-tile new windows：勾选后，该 app 之后**新开**的窗口自动加入网格（标题带 app 名，
        //     在 menuNeedsUpdate 里动态刷新）。字符串暂用英文硬编码，避免改动本地化表；本地化为后续项。
        menu.addItem(.separator())
        let tileNowItem = menu.addItem(withTitle: "Tile Now", action: #selector(tileNowAction), keyEquivalent: "t")
        tileNowItem.target = self
        tileNowItem.keyEquivalentModifierMask = [.command, .option]
        tileNowItem.image = NSImage(systemSymbolName: "rectangle.split.2x2", accessibilityDescription: nil)

        // 「Tile Layout」子菜单：可视化选择前台 app 的网格布局（hover 实时预览、点选记忆，每 app+窗口数）。
        let layoutController = TileLayoutMenuController(gridController: gridController)
        tileLayoutMenuController = layoutController
        menu.addItem(layoutController.menuItem)

        let gridManageItem = menu.addItem(withTitle: "Auto-tile new windows", action: #selector(toggleGridManaged), keyEquivalent: "g")
        gridManageItem.target = self
        gridManageItem.keyEquivalentModifierMask = [.command, .option]
        gridManageItem.image = NSImage(systemSymbolName: "square.grid.3x3.topleft.filled", accessibilityDescription: nil)
