...
final class WindowCenteringService {
    ...
    /// `detectWindowContextUsingCG` 可容忍的最大「空间调和误差」（rectMatchError 的四维绝对差之和，px）。
    /// 四个候选坐标空间里至少有一个应与 CG 观测到的 cocoaRect 近乎吻合（err≈0；Retina/缩放的取整
    /// 抖动通常 <16px）。若**最优**空间的误差仍超过此阈值，说明 CG bounds 与 AX rawPosition 根本对不上——
    /// 多半是 CG 匹配到了另一个窗口/Space，或窗口在 AX 读取与 CG 读取之间移动过。此时信任 CG 会喂错
    /// 坐标空间，令平铺落位错位并反复触发 `tileReachedTarget: CG mismatch`。超阈值即退回纯 AX 上下文。
    /// 取值远低于日志中观测到的坏值（530 / 1512 / 3634），又高于正常取整抖动，两侧留足余量。
    nonisolated private static let maxReconcilableSpaceError: CGFloat = 64
    ...
    /// Plumb 自己刚落位过的窗口的短暂宽限截止（key = `pid:windowNumber`，与 Observer 的
    /// `key(pid:window:)` 同构）。部分 app（如 Finder）会在动画定时器移除**之后**才迟到投递
    /// 由我们写入 AXFrame 触发的 move/resize 通知——`isAnyAnimationInProgress` 此刻已为 false，
    /// 若不宽限，Observer 会把自铺窗口误标「手动」并在下次切 App 时丢弃（"网格铺完随即失管"的根因）。
    /// 网格路径（`placeWindowElementAnimated`）不经 Observer 的 selfLayoutGrace 机制，故在服务层
    /// 统一登记，由 Observer 的 move/resize 旁路一并查询。
    private var selfPlacementGraceUntil: [String: Date] = [:]
    private static let selfPlacementGraceInterval: TimeInterval = 1.8

    /// 是否有任意窗口动画正在进行中（供观察者决定是否需要重试 / 抑制手动标记）。
    /// **必须**涵盖 `activeAnimatorTimers` 与 `activeTileTimer`：网格的 `placeWindowElementAnimated`
    /// 只登记 `activeAnimatorTimers`（从不设 `activeAnimationKey`），旧实现仅看 `activeAnimationKey`
    /// 故对网格动画期间的自发 move/resize 视而不见 → 自铺窗口被误标「手动」。
    var isAnyAnimationInProgress: Bool {
        activeAnimationKey != nil || !activeAnimatorTimers.isEmpty || activeTileTimer != nil
    }
    ...
    // MARK: - 网格资格 / 自铺宽限（供 GridTilingController 与 Observer 共享）

    /// 该窗口是否处于原生全屏（独占 Space，AX 无法移动/缩放）。全屏窗口不能参与网格平铺——
    /// 计入网格只会预留一个永远填不上的空格（`place FAILED … fullscreenWindow`）。
    /// 复用私有 `isFullscreenWindow`（AXFullScreen 主信号 + 几何回退）。
    func isFullscreen(_ windowElement: AXUIElement) -> Bool {
        isFullscreenWindow(windowElement)
    }

    /// 该窗口是否与给定屏同屏。探测失败返回 `nil`——调用方据此「按同屏保留」，
    /// 避免旧实现「screenForWindow 返回 nil 即把窗口全部过滤空 → Tile now 无反应」的回归。
    func windowIsOnScreen(_ windowElement: AXUIElement, pid: pid_t?, screen: NSScreen) -> Bool? {
        guard let windowScreen = screenForWindow(windowElement, pid: pid) else { return nil }
        return displayID(for: windowScreen) == displayID(for: screen)
    }

    /// 登记一次「Plumb 自己刚落位」的宽限。`placeWindowElementAnimated` 落位时调用，
    /// 使 Observer 在其后 `selfPlacementGraceInterval` 内到达的 move/resize 不被误判为用户手势。
    func noteSelfPlacement(_ windowElement: AXUIElement, pid: pid_t?) {
        guard let pid, let k = selfPlacementKey(pid: pid, window: windowElement) else { return }
        let now = Date()
        selfPlacementGraceUntil = selfPlacementGraceUntil.filter { $0.value > now }
        selfPlacementGraceUntil[k] = now.addingTimeInterval(Self.selfPlacementGraceInterval)
    }

    /// 该窗口当前是否处于「自铺宽限」内（供 Observer 的 move/resize 旁路查询，与
    /// `isAnyAnimationInProgress` 互补：前者覆盖动画结束后迟到的通知，后者覆盖动画进行中）。
    func isWithinSelfPlacementGrace(_ windowElement: AXUIElement, pid: pid_t) -> Bool {
        let now = Date()
        selfPlacementGraceUntil = selfPlacementGraceUntil.filter { $0.value > now }
        guard let k = selfPlacementKey(pid: pid, window: windowElement), let until = selfPlacementGraceUntil[k] else {
            return false
        }
        return until > now
    }

    /// 与 Observer 的 `key(pid:window:)` 同构：优先 AXWindowNumber（跨进程稳定），缺失回退 CFHash。
    private func selfPlacementKey(pid: pid_t, window: AXUIElement) -> String? {
    ...
    /// 「Tile Layout」实时预览：抓取窗口当前的**原始 AX** frame（position+size，未做任何坐标空间转换）。
    /// 供预览取消时精确还原——原始值写回无需坐标空间推断，故还原无损、与探测无关。
    func rawAXFrame(of windowElement: AXUIElement) -> (position: CGPoint, size: CGSize)? {
        guard let p = pointAttribute(kAXPositionAttribute as CFString, on: windowElement),
              let s = sizeAttribute(kAXSizeAttribute as CFString, on: windowElement) else { return nil }
        return (p, s)
    }

    /// 把窗口还原到快照的原始 AX frame（先 size 后 position，与常规落位一致，减少 clamp 抖动）。
    func restoreRawAXFrame(_ frame: (position: CGPoint, size: CGSize), to windowElement: AXUIElement) {
        _ = setSizeAttribute(kAXSizeAttribute as CFString, value: frame.size, on: windowElement)
        _ = setPointAttribute(kAXPositionAttribute as CFString, value: frame.position, on: windowElement)
    }
    ...
    /// 把窗口瞬时放到一个**显式的目标 frame**（左下原点坐标系，位于窗口所在屏的可用区内）。
    ///
    /// 与 `tileWindowElement` 同构——同样的坐标空间探测 + resize + 逐空间兜底 + tileReachedTarget
    /// 验证——唯一区别是 targetFrame 由调用方（网格平铺）直接给出，而非从 insets 反算铺满整屏。
    /// 用于多窗口网格：调用方用 `WindowGeometry.gridFrames` 把区域切成 N 格，逐窗口调用本方法。
    ///
    /// ⚠️ 前提：targetFrame 必须是用**该窗口当前所在屏**的 `usableFrame(for:)` 算出来的
    /// （GridTilingController 已按参考屏过滤受管窗口）。跨屏放置不在 v1 范围内。
    func placeWindowElement(
        _ windowElement: AXUIElement,
        pid: pid_t? = nil,
        appElement: AXUIElement? = nil,
        targetFrame: CGRect
    ...
    /// `placeWindowElement` 的带动画版：从窗口当前 frame 平滑插值到显式目标 frame（位置+尺寸同时动），
    /// 与自动居中同一套 `WindowAnimator`（ease-out、丝滑）。用于网格平铺，让多窗口落位有动效，
    /// 观感与 Plumb 其余动画一致。
    ///
    /// 实现要点：
    ///   - 用**探测到的坐标空间**（context.space）一次性把目标算成 AX 目标 frame（不逐帧试空间）。
    ///   - 每帧写整帧：优先 AXFrame，回退 pos+size（Electron 类只吃 AXFrame，原生吃 pos+size）。
    ///   - 关闭拖动中止（reader 返回 nil）：网格动画同时改尺寸，中点漂移判据对 resize lag 会误判。
    ///   - 结束后再调一次瞬时 `placeWindowElement`（带逐空间验证+兜底）确保**精确**落位，
    ///     修正动画期间坐标空间猜测偏差 / app 尺寸滞后。
    ///   - 定时器登记进 `activeAnimatorTimers`：切 app 时 `abortActiveAnimations()` 一并取消，
    ///     避免 zombie 定时器在后台继续写非前台窗口（与居中/平铺动画同防线）。
    func placeWindowElementAnimated(
        _ windowElement: AXUIElement,
        pid: pid_t? = nil,
        appElement: AXUIElement? = nil,
        targetFrame: CGRect,
        ...
        let startAXFrame = CGRect(origin: currentPosition, size: windowSize)
        let targetAXOrigin = toAXOrigin(
            bottomLeftOrigin: targetFrame.origin,
            windowSize: targetFrame.size,
            screenFrame: context.screen.frame,
        ...
        let targetAXFrame = CGRect(origin: targetAXOrigin, size: targetFrame.size)

        // 已基本到位：直接瞬时精确落位，无需动画。
        if abs(startAXFrame.minX - targetAXFrame.minX) < 1, abs(startAXFrame.minY - targetAXFrame.minY) < 1,
           abs(startAXFrame.width - targetAXFrame.width) < 1, abs(startAXFrame.height - targetAXFrame.height) < 1 {
            try? placeWindowElement(windowElement, pid: pid, appElement: appElement, targetFrame: targetFrame)
            noteSelfPlacement(windowElement, pid: pid)   // 瞬时写入亦会触发迟到的自发 move/resize，需宽限。
            completion?()
            return
        }

        // 动画期间 `isAnyAnimationInProgress` 为真已覆盖同步通知；此处预登记宽限，覆盖
        // 动画定时器移除后仍迟到投递的通知（如 Finder）。完成回调会再刷新一次。
        noteSelfPlacement(windowElement, pid: pid)

        var timerBox: DispatchSourceTimer?
        timerBox = WindowAnimator.animate(
            from: startAXFrame,
            to: targetAXFrame,
            easing: WindowAnimator.easeOut,
            writer: { [weak self] frame in
                guard let self else { return false }
                if self.setRectAttribute("AXFrame" as CFString, value: frame, on: windowElement) {
                    return true
                }
                let okPos = self.setPointAttribute(kAXPositionAttribute as CFString, value: frame.origin, on: windowElement)
                let okSize = self.setSizeAttribute(kAXSizeAttribute as CFString, value: frame.size, on: windowElement)
                return okPos || okSize
            },
            reader: { nil },   // 关闭拖动中止（见方法注释）。
            completion: { [weak self] in
                guard let self else { completion?(); return }
                if let t = timerBox {
                    self.activeAnimatorTimers.removeAll { $0 === t }
                }
                // 精确落位兜底（逐空间验证 + AXFrame 兜底），修正动画期间的空间猜测/尺寸滞后。
                try? self.placeWindowElement(windowElement, pid: pid, appElement: appElement, targetFrame: targetFrame)
                // 定时器已在上一步移除 → isAnyAnimationInProgress 转 false；刷新宽限以覆盖此后迟到的自发通知。
                self.noteSelfPlacement(windowElement, pid: pid)
                completion?()
            }
        )
        if let t = timerBox {
            activeAnimatorTimers.append(t)
        }
    }

    /// 该窗口当前所在屏（供 GridTilingController 按参考屏过滤受管窗口，保证区域几何与写入同屏）。
    /// 探测失败 → nil。
    func screenForWindow(_ windowElement: AXUIElement, pid: pid_t?) -> NSScreen? {
        guard
            let currentPosition = pointAttribute(kAXPositionAttribute as CFString, on: windowElement),
            let windowSize = sizeAttribute(kAXSizeAttribute as CFString, on: windowElement)
        else { return nil }
        let primaryTopY = primaryScreenTopY()
        if let pid, let cg = detectWindowContextUsingCG(windowElement: windowElement, pid: pid, rawPosition: currentPosition, windowSize: windowSize, primaryTopY: primaryTopY) {
            return cg.screen
        }
        return detectWindowContext(rawPosition: currentPosition, windowSize: windowSize, pid: pid, primaryTopY: primaryTopY)?.screen
    }

    /// 窗口当前 frame（全局左下原点坐标系）。供网格「互补区」读取 focused window 的占用矩形。
    func globalFrame(of windowElement: AXUIElement, pid: pid_t?) -> CGRect? {
        guard
            let currentPosition = pointAttribute(kAXPositionAttribute as CFString, on: windowElement),
            let windowSize = sizeAttribute(kAXSizeAttribute as CFString, on: windowElement)
        else { return nil }
        let primaryTopY = primaryScreenTopY()
        if let pid, let cg = detectWindowContextUsingCG(windowElement: windowElement, pid: pid, rawPosition: currentPosition, windowSize: windowSize, primaryTopY: primaryTopY) {
            return cg.currentGlobalRect
        }
        return detectWindowContext(rawPosition: currentPosition, windowSize: windowSize, pid: pid, primaryTopY: primaryTopY)?.currentGlobalRect
    }

    /// 把窗口瞬时放到**指定屏**的显式目标 frame（全局左下坐标）。与 `placeWindowElement` 不同：
    /// 坐标空间从窗口探测（app 属性，跨屏一致），但转换用**传入的 targetScreen**（而非窗口当前所在屏）。
    /// 这让「把别屏 / 别处的窗口搬到聚焦显示器的网格格子」成立——多屏 / Luna Display 的关键。
    ///
    /// 逐候选空间写入并**按 targetScreen 验证**读回是否落到目标；成功即返回。全失败抛错。
    /// ⚠️ AX 尺寸写入仅在目标 app 为前台时生效——调用方（GridTilingController）在写入前会 activate 该 app。
    func placeWindowOnScreen(
        _ windowElement: AXUIElement,
        pid: pid_t? = nil,
        appElement: AXUIElement? = nil,
        targetFrame: CGRect,
        ...
        let primaryTopY = primaryScreenTopY()
        // 探测坐标空间（用于优先顺序）；探测失败则从常见 globalTopLeft 起试。
        let detectedSpace: RawSpace = {
            if let pid, let cg = detectWindowContextUsingCG(windowElement: windowElement, pid: pid, rawPosition: currentPosition, windowSize: windowSize, primaryTopY: primaryTopY) {
                return cg.space
            }
            return detectWindowContext(rawPosition: currentPosition, windowSize: windowSize, pid: pid, primaryTopY: primaryTopY)?.space ?? .globalTopLeft
        }()
        ...
        let spaces = prioritizedSpaces(primary: detectedSpace)

        func matches(space: RawSpace) -> Bool {
            guard
                let p = pointAttribute(kAXPositionAttribute as CFString, on: windowElement),
                let s = sizeAttribute(kAXSizeAttribute as CFString, on: windowElement)
            else { return false }
            let global = rawToGlobalRect(space: space, screenFrame: screenFrame, rawPosition: p, windowSize: s, primaryTopY: primaryTopY)
            return abs(global.minX - targetFrame.minX) <= 8 && abs(global.minY - targetFrame.minY) <= 8
                && abs(global.width - targetFrame.width) <= 8 && abs(global.height - targetFrame.height) <= 8
        }

        for space in spaces {
            let origin = toAXOrigin(bottomLeftOrigin: targetFrame.origin, windowSize: targetFrame.size, screenFrame: screenFrame, space: space, primaryTopY: primaryTopY)
            if setPointAttribute(kAXPositionAttribute as CFString, value: origin, on: windowElement), matches(space: space) {
                return
            }
        }
        for space in spaces {
            let origin = toAXOrigin(bottomLeftOrigin: targetFrame.origin, windowSize: targetFrame.size, screenFrame: screenFrame, space: space, primaryTopY: primaryTopY)
            let frameRect = CGRect(origin: origin, size: targetFrame.size)
            if setRectAttribute("AXFrame" as CFString, value: frameRect, on: windowElement), matches(space: space) {
                return
            }
        }
        throw WindowCenteringError.unableToWriteWindowPosition
    ...
    private func detectWindowContextUsingCG(
        ...
        // CG 与 AX 无法调和（最优空间误差仍过大）→ 不信任本次 CG 探测，退回纯 AX 上下文。
        // 返回 nil 让每个调用点走其既有的 `detectWindowContext` 兜底分支（与"无屏幕录制权限/CG 取
        // 边界失败"完全同路径，久经验证）。此处**不**写缓存，避免把坏空间污染 cachedSpaceByPID——
        // 函数开头的 fallbackContext(=detectWindowContext) 已把正确的 AX 空间写入缓存。
        if best.error > Self.maxReconcilableSpaceError {
            DiagnosticLog.debug("detectCG: CG/AX irreconcilable (err=\(best.error) > \(Self.maxReconcilableSpaceError)) → AX fallback pid=\(pid)")
            return nil
        }
    ...
    private func cgWindowBounds(
        ...
        // Some apps do not expose AXWindowNumber (e.g. certain Office windows, and —
        // critically — **Finder**, whose window keys are all `pid:ax:…` CFHash fallbacks).
        // Fallback: pick the best on-screen window for this PID by closest size.
        guard let list = CGWindowListCopyWindowInfo([.optionOnScreenOnly], kCGNullWindowID) as? [[String: Any]] else {
            return nil
        }

        // Collect this PID's standard (layer-0) windows.
        ...
        // Size-matching is only safe when it's UNAMBIGUOUS — i.e. the app has a single
        // standard window. With 2+ windows and no windowID to disambiguate (Finder!),
        // the closest-size guess routinely returns the WRONG window: several windows share
        // a size, and sizes shift mid-tiling-animation. That returns another window's bounds,
        // which corrupts coordinate-space detection and sends windows to the wrong cells.
        // Bail → caller falls back to pure-AX detection (per-window, no cross-window mixup).
    ...
    /// 该 pid **当前 Space 上、可见**（未最小化、layer 0 标准窗口）的窗口 frame（左下原点全局坐标），
    /// 连同各窗口所在**显示器的 frame**（供按聚焦屏过滤）。用 CGWindowList 的 `.optionOnScreenOnly`——
    /// 它只列出当前 Space 里正在渲染的窗口，天然排除其它 Space、最小化、以及 off-screen 的窗口。供
    /// GridTilingController 稳定判定「屏上到底有几个窗口」（AX 的 kAXWindows 会把别的 Space / 最小化窗口
    /// 也算进来，且 AX 屏归属探测不稳 → 数量在 6~8 间抖）。需屏幕录制权限；不可用则返回空（调用方回退不过滤）。
    func onScreenWindowFrames(pid: pid_t) -> [(frame: CGRect, screenFrame: CGRect)] {
        guard ScreenCapturePermission.ensureAuthorized(prompt: false),
              let list = CGWindowListCopyWindowInfo([.optionOnScreenOnly], kCGNullWindowID) as? [[String: Any]]
        else { return [] }
        let primaryTopY = primaryScreenTopY()
        var result: [(frame: CGRect, screenFrame: CGRect)] = []
        for info in list {
            guard let owner = info[kCGWindowOwnerPID as String] as? Int, owner == Int(pid) else { continue }
            guard let layer = info[kCGWindowLayer as String] as? Int, layer == 0 else { continue }
            guard let bd = info[kCGWindowBounds as String] as? NSDictionary,
                  let cg = CGRect(dictionaryRepresentation: bd),
                  let pick = pickScreenForCGRect(cg) else { continue }
            result.append((cocoaRectFromCGWindowBounds(cg, screen: pick.screen, primaryTopY: primaryTopY), pick.screen.frame))
        }
        return result
    }
    ...
    /// 公开包装：某屏的可用区（已排除 Dock / 菜单栏），左下原点坐标系。
    /// 供网格平铺（GridTilingController）计算区域网格——与本服务写入路径共用同一套
    /// 「可用区」定义，保证区域几何与坐标写入一致。
    func usableFrame(for screen: NSScreen) -> CGRect {
        effectiveVisibleFrame(for: screen)
    }
