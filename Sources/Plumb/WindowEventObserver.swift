...
final class WindowEventObserver {
    ...
    /// 网格平铺编排器（可选注入）。非 nil 且某 app 被标记为网格受管时，该 app 的窗口事件
    /// 被 handle() 提前分流到网格路径（接管排版，不再走自动居中/最大化式平铺）。
    private let gridController: GridTilingController?

    init(service: WindowCenteringService, tilingSettingsStore: AppTilingSettingsStore, dmgMonitor: DmgMountMonitor? = nil, gridController: GridTilingController? = nil) {
    ...
    @objc private func appDidTerminate(_ notification: Notification) {
        ...
        processedPIDs.remove(pid)
        endDocumentStableGate(pid: pid)
        // 网格受管状态随 app 退出丢弃（避免 PID 复用串味）。
        gridController?.appTerminated(pid: pid)
    }
    ...
    private func handle(notification: String, element: AXUIElement, forcedPID: pid_t? = nil) -> Bool {
        ...
        // 网格平铺分流：若该 app 被标记为网格受管，由 GridTilingController 接管排版——
        // 不走下方的自动居中 / 最大化式平铺。语义（用户确认的默认取舍）：
        //   - 新建窗口（kAXWindowCreated）→ 只把新窗口加入受管集合并重排（不扫入已存在窗口）。
        //   - 焦点变化 / initial-retry → reconcile（修剪已关闭窗口后回填），**不**扫入已存在窗口。
        //   - 「Tile now」（菜单/热键）是把已存在窗口纳入的唯一途径，走 AppDelegate → controller，不经此。
        // 返回 true：让 initial-retry 循环停止（受管 app 不需要重复的居中重试）。
        if let gridController, gridController.isManagedApp(bundleIdentifier: frontmostApp.bundleIdentifier) {
            let gridAppElement = AXUIElementCreateApplication(pid)
            let bundleID = frontmostApp.bundleIdentifier
            if notification == (kAXWindowCreatedNotification as String) {
                gridController.handleWindowCreated(pid: pid, appElement: gridAppElement, createdElement: element, bundleIdentifier: bundleID)
            } else {
                // focusedChanged / initial-retry / 其它：只 reconcile，不扫入已存在窗口。
                gridController.handleReconcile(pid: pid, appElement: gridAppElement, bundleIdentifier: bundleID)
            }
            return true
        }
    ...
    private func handleResize(element: AXUIElement, forcedPID: pid_t? = nil) -> Bool {
        ...
        // 网格/居中动画落位后迟到的自发 resize（服务层统一登记）——不视作用户手势。
        if service.isWithinSelfPlacementGrace(element, pid: pid) {
            DiagnosticLog.debug("handle[resize]: self-placement grace (grid/animated) — ignore, no manual mark pid=\(pid)")
            return false
        }
    ...
    private func handleMove(element: AXUIElement, forcedPID: pid_t? = nil) -> Bool {
        ...
        // 网格/居中动画落位后迟到的自发 move（服务层统一登记）——不视作用户手势。
        if service.isWithinSelfPlacementGrace(element, pid: pid) {
            DiagnosticLog.debug("handle[move]: self-placement grace (grid/animated) — ignore, no manual mark pid=\(pid)")
            return false
        }
