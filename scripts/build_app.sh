...
# 用 `-p codesigning` 策略查询：自签名身份在 macOS 15/26 上不被默认 X.509 策略信任
# （`find-identity -v` 会漏列，报 CSSMERR_TP_NOT_TRUSTED），但对**代码签名**策略有效——
# codesign 本身也只认这个策略。用默认 `-v` 会误判为「身份缺失」而回退 ad-hoc，白白丢掉
# TCC 跨更新保留（自签名身份的 designated requirement 锚定 leaf 证书 hash，正是 TCC 所需）。
