...
# 1) Idempotency: check for an identity valid under the CODESIGNING policy.
#    (Plain `find-identity -v` uses the default X.509 policy, under which a self-signed
#    cert reads as CSSMERR_TP_NOT_TRUSTED on macOS 15/26 and is omitted — that false
#    negative made re-runs regenerate a duplicate cert. `-p codesigning` is what both
#    codesign and build_app.sh actually rely on, so match it here.)
...
# 6) VERIFY the identity is now usable for CODE SIGNING (the only thing that matters
#    for a stable, TCC-preserving signature). We check `-p codesigning` rather than the
#    default policy: on macOS 15/26 a self-signed cert stays CSSMERR_TP_NOT_TRUSTED under
#    the default X.509 policy even after admin-domain trust, but codesign still accepts it
#    and produces a leaf-anchored designated requirement. The old default-policy check
#    false-failed here even though the identity was perfectly usable.
if ! security find-identity -v -p codesigning | grep -q "\"${CERT_NAME}\""; then
  echo ""
  echo "❌ 证书已导入但对代码签名策略仍不可用（find-identity -v -p codesigning 未列出该身份）。"
  echo "   请在「钥匙串访问」App 中手动信任该证书（双击 → 信任 → 代码签名 → 始终信任），"
